...
from header import plot_trajectory, normfac, n20_to_azz, azz_to_n20, FS
...
# Generate model lines from scratch? Else load saved Pickle files.
INTEGRATE_MODEL = 1
...
# Number of integration steps for model trajectories (should be large enough to reach steady state)
Nt = 300
...
def integrate_model(nlm0, ugrad, dt, reg="hyper", Nt=Nt, rotate=False, name=None):
    ...
    with Bar("dt=%.3e, Nt=%i :: L=%i (nlm_len=%i) ::" % (dt, Nt, L, nlm_len), max=Nt - 1, fill="#", suffix="%(percent).1f%% - %(eta)ds") as bar:
        for tt in np.arange(1, Nt):
            ...
            # note that CDRX is identical the Laplacian operator, so used as regularization here
            M_REG = sf.M_REG(nlm_prev, eps) if reg == "hyper" else 10e-2 * sf.M_CDRX(nlm_prev)
            M = M_LROT + M_REG
    ...
    if name is not None:
        pickle.dump([nlm, lm, nlm_len], open(pfile(name), "wb"))
...
if INTEGRATE_MODEL:
    ...
    nlm_iso[0] = normfac
    nlm_uc20 = integrate_model(nlm_iso, +np.diag([0.5, 0.5, -1]), 0.015 * 1, name="uc20")  # unconfined compression
    nlm_ue20 = integrate_model(nlm_iso, -np.diag([0.5, 0.5, -1]), 0.007 * 2, name="ue20")  # unconfined extension
    ...
    nlm_uc6 = integrate_model(nlm_iso, +np.diag([0.5, 0.5, -1]), 0.015 * 1, name="uc6")
    nlm_ue6 = integrate_model(nlm_iso, -np.diag([0.5, 0.5, -1]), 0.007 * 2, name="ue6")  # unconfined extension

    nlm_uc6_lap = integrate_model(nlm_iso, +np.diag([0.5, 0.5, -1]), 0.015 * 1, name="uc6lap", reg="lap")  # unconfined compression
    nlm_ue6_lap = integrate_model(nlm_iso, -np.diag([0.5, 0.5, -1]), 0.007 * 2, name="ue6lap", reg="lap")  # unconfined extension
...
scale = 3.5
fig = plt.figure(figsize=(1 * 1.7 * scale, 1.5 * scale))
...
manual_locations = [(-0.3, 1.5)] + [(0.51, y_) for y_ in np.linspace(-1.4, 2.1, len(lvls[1:]))]
manual_locations[3] = (-1.0, manual_locations[3][1] - 0.5)
...
legend_strings = ["Unconf. compr., L=20", "Unconf. compr., L=6", "Unconf. compr., L=6, no hyper dif.", "Unconf. exten., L=20", "Unconf. exten., L=6", "Unconf. exten., L=6, no hyper dif."]

legend_modellines = ax.legend(h_modellines, legend_strings, loc=3, bbox_to_anchor=(-0.17, -0.5), ncol=2, fontsize=FS - 2, frameon=True, **legkwargs)
legend_modellines.set_title("Modeled fabric-state trajectories", prop={"size": FSLEG, "weight": "bold"})
...
dytext = 0.04 / normfac
ax.text(0, 0 - dytext, "Isotropic", weight="bold", color="k", ha="center", va="top", fontsize=FSANNO)
...
ax.text(n20_delta - 0.5 * dytext, n40_delta + dytext, "Unidirectional", weight="bold", color=c_smax, ha="center", va="bottom", ma="center", fontsize=FSANNO)
...
if 1:
    ...
    ind = np.argmin(np.abs(X - n20_delta))
    badspec[:, ind:] = 0
    ind = np.argmin(np.abs(Y - n40_delta))
    badspec[ind:, :] = 0
    lvls = [0, 0.5]
    ...
    cs = ax.contour(
        ...
        linewidths=plt.rcParams["hatch.linewidth"],
        colors=plt.rcParams["hatch.color"],
    )
    ax.text(-1.2, 3.3, "Restricted", weight="bold", fontsize=FS - 2, color="0.1")
else:
    lw = 1.1
...
# Save figure
plt.savefig("../plots/figure_10.png", dpi=300)
plt.close()
