...
program demo
    ...
    real(kind=dp)      :: nu0  = 2.0e-2 ! CDRX per year at -5C




    ! Fabric state and evolution
    complex(kind=dp), allocatable :: nlm(:), dndt(:,:), dndt_ROT(:,:), dndt_REG(:,:), dndt_CDRX(:,:), dndt_DDRX(:,:) ! Series expansion coefs and evolution matrix
    ...
    allocate(dndt_CDRX(nlm_len,nlm_len))    
    ...
    dndt_ROT = M_LROT(eps, omg, 1.0_dp, 0.0_dp) ! Assume constant strain-rate and spin with Taylor style plastic spin for lattice rotation (beta=1).
    dndt_REG = M_REG(eps)
    dndt_CDRX = nu0 * M_CDRX()

    do tt = 2, Nt
!        write(*,"(A9,I3)") '*** Step ', tt
        dndt_DDRX = GammaNaught *M_DDRX(nlm, eps) ! Tau is assumed constant, but since DRX rate depends on the state itself (i.e.
        dndt = dndt_ROT + dndt_REG + dndt_DDRX + dndt_CDRX
    ...
    write (fname_sol,"('solutions_L20/FULL_',A5,'.nc')") arg_exp
    ...
    print *, "Plot result:"
    write(*,"(A28,A5)") "python3 plot_demo_FULL.py ", arg_exp
