...
def interpolate(x, y, x0, y0, q):
    ...
    p = (q[i, j] + (x0 - x[j]) * (q[i, j + 1] - q[i, j]) / dx + (y0 - y[i]) * (
        q[i + 1, j] - q[i, j]) / dy + (x0 - x[j]) * (y0 - y[i]) * (
            q[i + 1, j + 1] + q[i, j] - q[i + 1, j] - q[i, j + 1]) / (dx * dy))
    return p


def streamline(x, y, vx, vy, x0, y0, stepsize=50.0,
               minvel=0.01, timesize=None, maxpts=1e4, trackbetter=True):
...
def make_centerlines(x, y, vx, vy, xs, ys, maxpts_down=750, maxpts_up=45, mv=0.01):
    ...
    """
    X1, Y1, V1, age1 = streamline(x, y, vx, vy, xs, ys, stepsize=50.0,
                                  minvel=mv, timesize=None, maxpts=maxpts_down)
    X2, Y2, V2, age2 = streamline(x, y, -vx, -vy, xs, ys, stepsize=50.0,
                                  minvel=mv, timesize=None, maxpts=maxpts_up)
    X = np.hstack((np.flip(np.asarray(X2))[:-1], np.asarray(X1)))
    Y = np.hstack((np.flip(np.asarray(Y2))[:-1], np.asarray(Y1)))
    age = np.hstack((np.flip(np.asarray(age2))[:-1],
                     np.asarray(age1) + age2[-1]))
    rec = np.c_[X[::-1], Y[::-1]]
    dist_center = [0.]
    for i in range(1, len(X)):
        dist_center.append(
            dist_center[i - 1] + np.sqrt(
                (rec[i][0] - rec[i - 1][0]) ** 2 + (
                    rec[i][1] - rec[i - 1][1]) ** 2))
...
def get_transect(
    ...
    surf_fn = data_dir + \
        '/antarctica_general/REMA_100m_domec.tif'
    ...
    # at the ends
    line = cl.coords
    slope = np.hstack(((line[1, 1] - line[0, 1]) / (line[1, 0] - line[0, 0]),
                       (line[2::, 1] - line[:-2:, 1]) / (line[2::, 0] - line[:-2:, 0]),
                       (line[-2, 1] - line[-1, 1]) / (line[-2, 0] - line[-1, 0])))
    slope = -1.0 / slope

    # x_offset should be positive if
    ftw = 800.0
    x_sign = -np.hstack(
        ([-1.], np.sign(line[2:, 1] - line[:-2, 1]), [1.]))
    x_sign[x_sign == 0.] = 1.
    x_off = x_sign * np.sqrt(
        ftw ** 2.0 / (slope ** 2.0 + 1.0))
    y_off = x_sign * slope * np.sqrt(
        ftw ** 2.0 / (slope ** 2.0 + 1.0))
    ...
    mag = (center_off_x ** 2.0 + center_off_y ** 2.0) ** (1. / 2.)
    scale = 2000.0 / mag
    center_off_x = scale * center_off_x
    center_off_y = scale * center_off_y

    ldist, lcenterline, lage, _ = make_centerlines(
        x, y, vx, vy,
        cl.coords[ind_edc, 0] + center_off_x,
        cl.coords[ind_edc, 1] + center_off_y)

    rdist, rcenterline, rage, _ = make_centerlines(
        x, y, vx, vy,
        cl.coords[ind_edc, 0] - center_off_x,
        cl.coords[ind_edc, 1] - center_off_y)

    ldist, lcenterline2, lage, _ = make_centerlines(
        x, y, vx, vy,
        center_east[0] + center_off_x,
        center_east[1] + center_off_y)

    rdist, rcenterline2, rage, _ = make_centerlines(
        x, y, vx, vy,
        center_east[0] - center_off_x,
        center_east[1] - center_off_y)
    ...
    if debug:
        ...
        ll = plt.plot(lcenterline[:, 0],
                      lcenterline[:, 1],
                      linestyle='dotted')
        plt.plot(cl.coords[:, 0], cl.coords[:, 1])
        lr = plt.plot(rcenterline[:, 0],
                      rcenterline[:, 1],
                      linestyle='dashed')
    ...
    l_ends = offset * 10. + cl.coords
    r_ends = -offset * 10. + cl.coords
    if debug:
        plt.plot(l_ends[:, 0],
                 l_ends[:, 1])
        plt.plot(r_ends[:, 0],
    ...
    int_lines = [shapely.geometry.LineString([l_e, r_e])
                 for l_e, r_e in zip(l_ends, r_ends)]
    lints = [l_line.intersection(int_line) for int_line in int_lines]
    left_point = line - offset
    right_point = line + offset
    left_point = np.array([intl.coords[0]
                           if isinstance(intl, shapely.geometry.Point)
                           else left_point[j, :]
                           for j, intl in enumerate(lints)])

    rints = [r_line.intersection(int_line) for int_line in int_lines]
    right_point = np.array([intr.coords[0]
                            if isinstance(intr, shapely.geometry.Point)
                            else right_point[j, :]
                            for j, intr in enumerate(rints)])
    if debug:
        plt.plot(left_point[:, 0],
                 left_point[:, 1],
                 color=ll[0].get_color())
        plt.plot(right_point[:, 0],
                 right_point[:, 1],
                 color=lr[0].get_color())
        print('Intersections found')
        plt.axis('equal')
    ...
    bed_fn = data_dir + \
        '/antarctica_general/bedmachine_bed.tif'
    acc_fn = data_dir + \
        '/antarctica_general/snw_final.nc'
    ts_fn = data_dir + \
        '/antarctica_general/ts_mean.nc'
    ...
    fn_base = 'domec_v2'
    gmshlib.gmsh_outline(fn_base + '.geo',
                         [bottom[:-1, :], [bottom[-1, :]], top[:-1, :], [top[-1, :]]],
                         RES,
                         cuts=None,
                         cuts_lc=None,
                         points=None,
